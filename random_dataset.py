...
# image_transforms = A.Compose([A.Normalize(mean=[0,0,0],std=[1,1,1]),ToTensorV2()]) # Weights 4
# image_transforms = A.Compose([A.Normalize(mean=[0.5,0.5,0.5],std=[0.5,0.5,0.5]),ToTensorV2()]) # Weights 5
image_transforms = A.Compose([A.Normalize(mean=MEAN,std=STD),ToTensorV2()]) # Weights 6


# to_tensor = A.Compose([A.Normalize(mean=[0,0,0],std=[1,1,1]),ToTensorV2()]) # Weights 4
# to_tensor = A.Compose([A.Normalize(mean=[0.5,0.5,0.5],std=[0.5,0.5,0.5]),ToTensorV2()]) # Weights 5
to_tensor = A.Compose([A.Normalize(mean=MEAN,std=STD),ToTensorV2()]) # Weights 6
class ImageFolder(Dataset):
    ...
    def __getitem__(self,index):
        ...
        if self.lr_idx >= self.lr_len:
            self.lr_idx = 0
            np.random.shuffle(self.lr_shuf)
        lr_image = np.asarray(Image.open(self.lr_data[self.lr_shuf[self.lr_idx]]))
